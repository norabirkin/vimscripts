...
$kk=array("repnum" => 10,"ugroups"=>$usergroup_id,"agentid" => $numagent, "dtfrom" => $dtfrom, "dtto" => $dtto,"showdefault"=>1,"pgnum"=>$pgnum,"pgsize"=>$pgsize);
...
if ($_POST['filter']!='null'){
	...
	if(sizeof($_tmp) > 0) { 
		echo '({"total": ' . (integer)$count->total . ',"summa":'.$sum.',"above":'.$above.',"rent":'.$rent.', "results": ' . JEncode($_tmp, $lanbilling) . '})';
	}
