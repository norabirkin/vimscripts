<?PHP
/********************************************************************
   filename:    savecreated_ug.php
   modified:   August 17 2004 20:46:36.
   author:      LANBilling
...
*********************************************************************/
   printf("<table  bgcolor=ffffff align=center cellspacing=0 cellpadding=0 border=0  width=750 style=\"border: solid 1px #c0c0c0;border-bottom:0px;\">
         <tr><td colspan=3 align=center bgcolor=e0e0e0 style=\"border-bottom: solid 1px #c0c0c0;\" height=25><b>%s</b></td></tr>", SAVINGNEWUNION);



   //print_r($_POST);
   $qstring = sprintf("select max(group_id)+1 from groups");
   $qresult = mysql_query($qstring);
   $cur_row = mysql_fetch_row($qresult);
   $cur_ugid = $cur_row[0];

   if($cur_ugid == 0) $cur_ugid = 1;
   ...
   printf("<tr><td align=left width=50%% bgcolor=ffffff style=\"border-bottom: solid 1px #c0c0c0;\"><b><font class=z11 size=-1>&nbsp;&nbsp;&nbsp;%s %s</font></b></td>", USERGROUPSENGLISH.COLON, BEHAVIOURUG);
   $qstring = sprintf("insert into groups set group_id = %d, description = '%s', name = '%s', tar_id='%d', user_type='%d'",
   $cur_ugid,
   $_POST['ug_descr'],
   $_POST['ug_name'],
   $_POST['tariffs_ug'], 
