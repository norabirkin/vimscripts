...
// +----------------------------------------------------------------------+
//
// $Id: IT.php,v 1.8 2007-09-21 07:45:55 rezunenko Exp $
//
 ...
 * @version  $Id: IT.php,v 1.8 2007-09-21 07:45:55 rezunenko Exp $
 * @access   public
...
class HTML_Template_IT {
    ...
    function setOption($option, $value)
    {
        ...
        return $this->errorMessage(IT_UNKNOWN_OPTION) . ": '{$option}'";
    }
    ...
    function setOptions($options)
    {
        ...
        {
            if($error=$this->setOption($option, $value))
                return $error;
        }
    }
    ...
    function show_xls($block = "__global__") {
    ...
    function parse($block = "__global__", $flag_recursion = false)
    {
        ...
        if (!isset($this->blocklist[$block])) {
            return $this->errorMessage( IT_BLOCK_NOT_FOUND ) . '"' . $block . "'";
        }
	...
	/**
	 * Returns all variables for the current block
	 *
	 */
	function getCurrentVariables($block = "__global__")
	{
		return $this->blockvariables[$block];
	} // end getCurrentVariables()
	
	
	/**
	 * Parse variables according to localization
	 * @param	block name
	 */
	function syncBlockLocalize( $block = "__global__" )
	{
		foreach( $this->blockvariables[$block] as $_key => $_val )
		{
			if( defined($_key) )
				$this->setVariable($_key, constant($_key));
		}
	} // end syncBlockLocalize()
	
	
	/**
	 * Return parsed current block
	 * @var     block name
	 * @var     distroy blockdata cell
	 */
	function getCurrentBlock( $block = "__global__", $distroy = true)
	{
        ...
        if (
            ...
            foreach ($regs as $k => $match) {
                ...
                if (isset($this->blocklist[$blockname])) {
                    $this->err[] =  $this->errorMessage(IT_BLOCK_DUPLICATE ) . '"' . $blockname . "'";
